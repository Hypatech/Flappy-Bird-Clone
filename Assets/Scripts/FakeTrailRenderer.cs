...
public class FakeTrailRenderer : MonoBehaviour
{
public float lifetime = 5f; //lifetime of a point on the trail
 ...
 void Awake () {
     rig = rig ?? GetComponent<Rigidbody>();
     line = GetComponent<LineRenderer>();
     line.useWorldSpace = true;
     points = new List<Vector3>() { rig.position }; //indices 1 - end are solidified points, index 0 is always rig.position
 ...
 void Update () {
     ...
     //add new point
     if (points.Count < 2 || Vector3.Distance(rig.position, points[1]) > minimumVertexDistance) {
         //if we have no solidified points, or we've moved enough for a new point
         AddPoint(rig.position);
     }
 
     //update index 0;
     points[0] = rig.position;
